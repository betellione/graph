...
int main() {
  ...
  sf::RenderWindow window(sf::VideoMode(1920, 1080), "Graph Editor",
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      ...
      if (typingWeight && selectedEdgeId != -1 &&
          event.type == sf::Event::TextEntered) {
        char ch = static_cast<char>(event.text.unicode);
        if (ch >= '0' && ch <= '9') {
          weightInput += ch;
        } else if ((ch == '.' || ch == ',') &&
                   weightInput.find('.') == std::string::npos) {
          weightInput += '.';
        }
      }
    ...
    if (draggedNodeId != -1) {
      graph.nodes[draggedNodeId].position =
